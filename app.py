...
데이터구조 (무조건 이 JSON 형식과 키 조합을 지켜서 반환할 것):
{
  ...
  "visual_elements": {
    "subject": {
      "name": "string (이 이미지의 중심 피사체. 단, 합성할 실제 제품명이나 음식 패키지는 절대 적지 말고 '빈 무대', '빈 테이블과 소품', '인물 모델' 등과 같이 배경적인 요소만 기입할 것)",
      "action": "string (실사 모델이 포함된 경우에만 '빈 공간을 가리키는 동작', '바라보는 동작' 등을 명시. 제품을 먹거나 만지는 동작 절대 금지)",
      "material_texture": "string",
      "human_model": "string (실사 모델이 포함된 경우 모델의 연령, 성별, 스타일 명시. 배제된 경우 'none' 기입)",
      "product_presentation": "string (제품 이미지 합성을 위해 사전에 확보해야 하는 빈 공간(레이아웃)의 위치와 특징)"
    },
    "layout_config": {
...
"""
        ...
        human_model_instruction = "이 이미지는 나중에 실제 제품 사진을 합성할 '백그라운드 배경 이미지' 역할입니다. 생성될 이미지 자체나 JSON의 어떤 응답 필드('action', 'name' 등)에도 실제 브랜드의 제품명, 패키지, 글씨, 음식(ex: 라면, 과자 등)을 절대 포함하거나 묘사하지 마세요. 실사 인물 모델을 추가하되, 인물은 특정 제품을 들고 있지 않아야 하며 오직 '제품이 곧 합성될 허공의 빈 공간' 혹은 '빈 테이블'을 가리키거나 응시하는 포즈로 묘사해야 합니다." if req.include_human_model else "이 이미지는 나중에 실제 제품 사진을 합성할 '백그라운드 배경 이미지' 역할입니다. 따라서 생성될 내용(JSON의 'action', 'name' 등)이나 이미지 묘사에 제품의 패키지, 브랜드 텍스트, 실제 음식 요소(ex: 라면 가닥 등)가 절대 노출되어서는 안 됩니다. 제품이 돋보일 수 있는 예쁜 3D 빈 공간(stage)이나 인테리어 빈 테이블(empty table)만이 존재하는 깨끗한 배경 샷을 연출해야 합니다."
        messages = [
        ...
        if final_image_b64:
            if not final_image_b64.startswith("data:"):
                img_url_payload = f"data:image/jpeg;base64,{final_image_b64}"
            else:
                img_url_payload = final_image_b64
                
            messages[1] = {
                "role": "user",
                "content": [
                    {"type": "text", "text": f"브랜드명: {req.brand_name}\n제품 URL 또는 설명: {req.product_url}\n조건: {human_model_instruction}\n첨부(분석)된 이미지를 참고하여 위 정보를 바탕으로 구조화된 JSON 응답을 주세요."},
                    {"type": "image_url", "image_url": {"url": img_url_payload}}
                ]
            }
        ...
        if req.include_human_model and subject.get('human_model') != "none":
            ...
            gemini_prompt += "The model should be presenting, looking at, or pointing at an empty space designated for a product overlay. IMPORTANT: DO NOT draw the physical product, packaging, or brand text itself. Leave the focused area completely empty for us to composite the product later. "
        else:
            gemini_prompt += "No humans, no people, empty product podium, clean studio background with a blank space for placing a product. IMPORTANT: DO NOT draw the physical product, packaging, or brand text itself. Just the empty stage/pedestal. "
